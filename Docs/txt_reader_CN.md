...
BookService 的命令也按值入队。返回的 `book_service_event` 只包含会话、卡代次、页号、偏移、总页数和状态等小型值数据，经 `SystemEventDispatcher` 送达 ReaderApp。完整索引留在 SD 上，不进入事件或 UI 帧。

### 页面帧

阅读器调用 `ui_write_reader_frame()`，其写入回调同步把正文快照、翻页可用状态和 `menu_visible` 复制到 UI 帧池。提交队列传递帧句柄，渲染任务持有对应引用进行绘制；应用对象和写入回调的上下文指针不进入渲染队列。菜单切换复用稳定的正文快照，不把正在加载中的分页器内容交给 UI。
...
PaperMono 屏幕为 480 × 800 像素，底部状态栏位于 `y=[760,800)`。阅读内容区域为 `(0,0,480,760)`，没有常驻底部导航栏。文字区域为 `(24,16,432,720)`，行高 30 像素，每页最多 24 行；最后一行的行框结束于 `y=736`。`layout.hpp` 统一定义文字边界和行数，`ui_reader_text_layout()` 将同一宽度、行数及字体度量交给前台和后台分页器，绘制也使用这些参数。

底部导航栏原占用 80 像素高度，移除后该空间归入阅读内容区域，每页行数由 21 增至 24。返回操作放入顶部菜单，翻页由正文点击区域处理。

通用 `reader_page` 预留 32 条行记录和 2048 字节文本数组，文本数组含结尾的空字符。当行数或文本容量到达边界时，分页器记录下一字符在原文件中的位置，作为下一页入口。
...
输入沿 `InputService → UIInteractionRouter → ui_action_event → ReaderApp` 传递。ReaderApp 接收语义动作，不处理屏幕坐标。

下列坐标采用左闭右开区间，均不包含状态栏：

| 区域 | 范围 | 菜单隐藏时 | 菜单显示时 |
| --- | --- | --- | --- |
| 左区 | `x=[0,160), y=[0,760)` | 上一页 | 禁止翻页；顶部菜单内的 `<` 单独处理 |
| 中区 | `x=[160,320), y=[0,760)` | 显示菜单 | `y=[80,760)` 关闭菜单；顶部菜单空白区域不触发动作 |
| 右区 | `x=[320,480), y=[0,760)` | 下一页 | 禁止翻页 |
| 顶部菜单 | `x=[0,480), y=[0,80)` | 不存在 | 优先接收点击，阻止穿透正文 |
| `<` | `x=[0,160), y=[0,80)` | 不存在 | 返回文件管理器 |

按下只捕获命中区域，松开后的普通 click 才发出一个动作。点击结束时重新校验已呈现状态和命中区域；位移超过 20 像素、跨区域点击和长按不触发 Reader 动作。三个正文区域及 `<` 都不产生按压反色刷新。首页禁用上一页，末页禁用下一页，加载期间禁止继续翻页；菜单可在加载和错误页面中打开。

菜单状态由 ReaderApp 管理，`reader_view_state.menu_visible` 随帧传递，绘制器只读取视图数据。菜单只有隐藏和显示两种状态；每次切换提交稳定画面，避免逐帧滑动引起连续墨水屏刷新。菜单中的 `<` 左对齐，菜单区域和条目矩形由 PaperMono 布局函数定义。

绘制顺序是正文、菜单背景、菜单文字。菜单覆盖正文顶部，不改变文字区域、页首或分页参数，也不触发索引重建。关闭菜单时重新绘制正文，恢复被覆盖的文字。

菜单切换提交 `popup_changed`，UI 选择 `reader_menu` 区域及 `text` 波形；普通翻页使用 `reader_content`。菜单更新会在帧缓冲中重新合成正文，但常规情况下只向屏幕刷新顶部 80 像素。如果合并队列后正文与上次成功显示的正文不同，则扩大为完整阅读内容区域，避免遗漏页面更新。菜单与正文共用残影计数，累计第 20 次 text 刷新可升级为全屏 quality；状态栏清理或 HAL 的刷新升级也由原有策略处理。ReaderApp 不指定硬件刷新模式。
...
| `BOOK_PAGINATION_VERSION` | `2` | 字体度量、正文布局及分页规则；版本不等时丢弃旧索引和旧进度，从第一页重建。 |

分页版本 2 对应移除底部导航栏后的正文高度。JSON 结构及 48 字节索引头、64 位偏移编码未改变，因此 schema 和索引格式版本仍为 1。即使 TXT 的大小、mtime 和三个 fingerprint 都相同，分页版本 1 的缓存也不能复用；其 `progress.page` 和 `progress.byte_offset` 均不用于恢复。
...
这里的“书签”是一份文件级自动续读位置，没有独立命名书签列表。有效索引恢复时优先使用 `progress.page` 查询页首。分页版本相同、文件内容判定相同，但索引缺失或损坏时，可使用有效的 `byte_offset`，在重建后的索引中二分查找不大于该偏移的最大页首。分页版本不同时不走该恢复路径：旧页号、字节偏移和索引均被弃用，打开第一页并从头建立索引。
...
2. 文件大小不同则重建。
3. 版本及索引头有效、大小相同且 mtime 相同，进入索引结构验证，不读取 TXT fingerprint。分页版本不同则从头构建，并重新计算 fingerprint。
...
BookService 使用名为 `book_worker`、stack 为 12288 字节、优先级为 `2` 的工作任务；前台存储工作任务的优先级为 `4`。索引引擎每次推进一页或一个块，处理后让出执行；缓冲内剩余字节可继续用于下一页。
...
主要固定内存包括：4096 字节扫描缓冲、4096 字节 JSON 缓冲、512 字节偏移缓冲、一个分页器、12288 字节任务栈及固定命令/事件队列。ESP32-S3 构建中索引引擎对象为 12168 字节。完整页表不驻留 RAM；一万页的索引占 `48 + 10000 × 8 = 80048` 字节，另加一份 metadata，重建期间还有临时文件占用。
...
Reader 的运行状态在打开和关闭时整体按值重置。`session_state` 的打开前字段只保存已校验的启动参数，并在打开时跨越整体重置复制一次；随后同一结构标识活动文件和介质。`page_state` 保存页面位置和视图状态；`content_request_state` 保存当前正文读取；`book_state` 协调持久化与索引；`cover_state`、`navigation_state` 和 `presentation_state` 分别管理封面、导航和帧提交生命周期。`session_serial_` 位于重置结构之外，因此延迟结果不会与新会话匹配。

正文左、右区分别控制上一页和下一页，中区控制顶部菜单。菜单显示时只能通过 `<` 返回或通过菜单下方的中区关闭菜单，左右区不翻页。墨水屏刷新方式由 UI 调度与刷新策略决定。
