...
4. BookIndexService 在独立工作任务中检查 SD 上的 metadata 与索引。分页版本相同且续读信息有效时，阅读器恢复到保存位置；分页版本不同时丢弃旧索引和旧进度，从第一页重新建立索引，不使用旧页码或字节偏移。用户已经主动翻页后，后台结果不再把视图跳回打开时的位置。
5. 每页正文仍按需读取。完整索引验证通过后，阅读器可以按页号查询字节偏移，并在状态栏显示当前页及总页数。

点击正文中区可打开顶部菜单，点击菜单左侧的 `Back` 调用 `app_request_back()`。返回操作由应用运行时处理。关闭阅读器后，文件管理器恢复原目录及列表页，并重新获取目录内容。每次打开和关闭 Reader 都将菜单状态重置为隐藏。
...
| 区域 | 范围 | 菜单隐藏时 | 菜单显示时 |
| --- | --- | --- | --- |
| 左区 | `x=[0,160), y=[0,760)` | 上一页 | 禁止翻页；顶部菜单内的 Back 单独处理 |
| 中区 | `x=[160,320), y=[0,760)` | 显示菜单 | `y=[80,760)` 关闭菜单；顶部菜单空白区域不触发动作 |
| 右区 | `x=[320,480), y=[0,760)` | 下一页 | 禁止翻页 |
| 顶部菜单 | `x=[0,480), y=[0,80)` | 不存在 | 优先接收点击，阻止穿透正文 |
| Back | `x=[0,160), y=[0,80)` | 不存在 | 返回文件管理器 |

按下只捕获命中区域，松开后的普通 click 才发出一个动作。点击结束时重新校验已呈现状态和命中区域；位移超过 20 像素、跨区域点击和长按不触发 Reader 动作。三个正文区域及 Back 都不产生按压反色刷新。首页禁用上一页，末页禁用下一页，加载期间禁止继续翻页；菜单可在加载和错误页面中打开。

菜单状态由 ReaderApp 管理，`reader_view_state.menu_visible` 随帧传递，绘制器只读取视图数据。菜单只有隐藏和显示两种状态；每次切换提交稳定画面，避免逐帧滑动引起连续墨水屏刷新。菜单中的 Back 左对齐，菜单区域和条目矩形由 PaperMono 布局函数定义。
...
正文左、右区分别控制上一页和下一页，中区控制顶部菜单。菜单显示时只能通过 Back 返回或通过菜单下方的中区关闭菜单，左右区不翻页。墨水屏刷新方式由 UI 调度与刷新策略决定。
