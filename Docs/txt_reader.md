...
3. `reader_app::on_open()` 建立阅读会话、清空回退历史、获取排版参数，并同时提交前台正文读取和 BookService 打开请求。前台先从偏移 `0` 生成可见页面，不等待全书扫描。
...
| `app` | 管理阅读行为与内容状态 | `reader_app` 维护会话、请求、翻页、菜单和错误状态，保留供菜单切换使用的正文快照；调用共享分页器解码并分页；应用运行时负责启动参数和返回导航。 |
